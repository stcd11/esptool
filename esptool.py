...
class CesantaFlasher(object):
    ...
    def __init__(self, esp, baud_rate=0):
        ...
        if baud_rate > 0:
            ...
            esp._port.flushOutput()

        # Flasher stub sleeps 5ms before listening, so we need
        # to sleep at least that long
        time.sleep(0.05)

        esp.write(struct.pack('<B', self.CMD_PING))
        p = esp.read()
        if p != 'OHAI':
            raise FatalError('Failed to ping the running flasher stub (got %s). Try lower baud rate?' % hexify(p))
        self.check_status_code(esp.read(), "Ping")
    ...
    def flash_read(self, addr, length, show_progress=False):
        ...
        if digest != expected_digest:
            raise FatalError('Digest mismatch: expected %s, got %s' % (expected_digest, digest))
        self.check_status_code(self._esp.read(), "Write")
        return data
    ...
    def flash_digest(self, addr, length, digest_block_size=0):
        ...
        while True:
            ...
            if len(p) == 16:
                ...
                self.check_status_code(p, "Digest")
                break
            else:
    ...
    def boot_fw(self):
        ...
        self.check_status_code(self._esp.read(), "Boot")

    def check_status_code(self, msg, operation):
        if len(msg) != 1:
            raise FatalError('Expected %s status, got: %s' % (operation, hexify(msg)))
        status_code = struct.unpack('<B', msg)[0]
        if status_code != 0:
            raise FatalError('%s failure, status: %x' % (operation, status_code))
...
# see flasher_stub/README.md for details.
...
042A21454D003198FF222E1D1A33380337B202C6D6FF2C02019FFFC000002191FF318CFF1A223A31\
019CFFC00000218DFF1C031A22454A000C02060300003C528601003C624600003C72918BFF9A1108\
...
053B002642164613000041CAFF222C1A1A444804202FC047328006F6FF222C1A273F3861C2FF222C\
...
C0000008712D0CC86112C1200DF00080FE3F4F4841498449004012C1D0C9A109B17CFC22C11132A0\
01051D00261202063700220111C2411032A00827B30286300031F3FF3022A0222200A0020021F1FF\
32A0042261001021208518000C020604002D011C0345190066820C2801381145A4FF224110C62200\
3C1206FDFF10212032A01045170066A20C2221003811482145B1FFC6F6FF4C1286F5FF002D011C03\
85150066B20F280138114821522103C5CDFF06EFFF005C1286EDFF0010212032A01045130066A20D\
2221003811482145DFFF06E7FF0022A06146E5FF85EEFFC6E3FF000001CCFFC0000006E1FF000C02\
2241100C1322C110850E0022011106070000000022C1100C13101120450D0022011132C2F9303074\
B6230206C1FF08B1C8A112C1300DF00000000000104088130000007519031027000000110040A810\
0040BC0F0040583F0040CC2E00401CE20040D83900408000004021F4FF12C1E0C961C80222A00002\
6107C90101F5FFC0000001F5FFC00000AC3C21EDFF01F3FFC0000021EBFFC031412A23C03C2001F0\
FFC000003D0222A00001EEFFC0000085E7FF20C22021E4FF01E8FFC00000667C202D0131E1FF8600\
00004B22C0200048023794F531DEFFC0200039023DF08601000001E1FFC000000871C86112C1200D\
F00012C1F002610301E8FEC00000083112C1100DF000643B004012C1D0E98109B1C9A1D991F97129\
013911E2A0C001FAFFC00000CD02E792F40C0DE2A0C0F2A0DB860D00000001F4FFC00000204220E7\
1240F7921C22610201EFFFC0000052A0DC482157120952A0DD571205460500004D0C3801DA234242\
001BDD3811379DC5C6000000000C0DC2A0C001E3FFC00000C792F608B12D0DC8A1D891E881F87112\
C1300DF00000", "entry": 1074792212, "num_params": 1, "params_start": 1074790400,\
 "data": "070610401B061040370610405606104076061040960610409E061040A8061040A80610\
40", "data_start": 1073643520}\
"""
